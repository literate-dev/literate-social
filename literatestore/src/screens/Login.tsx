...
const { width, height } = Dimensions.get('window');
 console.log(REACT_APP_GOOGLE_WEB_CLIENT_ID)
const schema = yup
  .object({
    email: yup
      .string()
...
const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      Toast.show({type:"error", text1: _.get(err,'code',''), text2: _.get(err,'message','')})
      console.log(err, statusCodes, 'error...')
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ImageBackground
        blurRadius={6}
        source={{uri: "https://getwallpapers.com/wallpaper/full/c/1/6/6346.jpg"}}
        ...
        <View style={styles.loginFormContainer}>
          ...
          <View style={styles.formContainerBlur}>
            ...
            <TouchableOpacity
              ...
              disabled={loading}>
              <Text style={styles.registerNow}>Register now ? click here</Text>
            </TouchableOpacity>
          </View>
          <ButtonComponent
            text="Login"
          ...
          <View style={styles.socialIconsContainer}>
            {[
              {
                icon: <Image source={require("../assets/googleIcon.png")} style={[{...styles.socialIcons, height: 30, width: 30, marginTop: 10}]}/>,
                name: "google"
              },
              {
                icon: <Image source={require("../assets/ios.png")} style={[{...styles.socialIcons, height:40, width:40}]}/>,
                name: "apple"
              },
              {
                icon: (
                  <Image source={require("../assets/facebook.png")} style={styles.socialIcons}/>
                ),
                name: "facebook"
              },
            ].map((each, idx) => (
              <TouchableOpacity key={idx} onPress={() => handleSocialLogin(each.name)}>{each.icon}</TouchableOpacity>
            ))}
          </View>
        </View>
      </ImageBackground>
